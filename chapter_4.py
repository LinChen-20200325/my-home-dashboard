"""Chapter 4：學長 — 貸款槓桿與科目四過水密技

獨立模組：提供 render_chapter_4() 函式，可被主程式 (main.py) import 使用。
...
def _render_gap_tab() -> None:
    ...
    # 核心計算（單位：萬）
    unit_gap_wan = max_unit_price_wan - target_unit_price_wan
    potential_gap_wan = unit_gap_wan * ping_size
    ...
    m1.metric(
        "同棟最高總價",
        f"{total_max_price_wan:,.1f} 萬",
        help=f"{max_unit_price_wan:.1f} 萬／坪 × {ping_size:.1f} 坪",
    )
    m2.metric(
        "你的目標總價",
        f"{total_target_price_wan:,.1f} 萬",
        help=f"{target_unit_price_wan:.1f} 萬／坪 × {ping_size:.1f} 坪",
    )
    m3.metric(
        "單坪價差",
        f"{unit_gap_wan:+.1f} 萬／坪",
        delta=(
            f"{(unit_gap_wan / max_unit_price_wan * 100):.1f}%"
            if max_unit_price_wan > 0
            else None
        ),
    )
    ...
    st.metric(
        "💰 預估未來可超額增貸空間",
        f"{potential_gap_wan:,.1f} 萬",
        delta=(
            f"{(potential_gap_wan / total_target_price_wan * 100):.1f}% of 出價"
            if total_target_price_wan > 0
            else None
        ),
        help="交屋後 6 個月以上向銀行申請增貸時，可能拗出來的合法現金。",
    )

    if potential_gap_wan > 0:
        st.info(
            "💡 **銀行估價看整棟！** 只要你買得比高價戶便宜，"
            "這個價差就是你交屋後能從銀行『搬出來的合法現金』！\n\n"
            f"以你目前的數字：交屋滿 6 個月後申請增貸，"
            f"有機會合法拿出 **{potential_gap_wan:,.1f} 萬**，"
            "再轉投入下一間或做其他金融操作。"
        )
    elif potential_gap_wan == 0:
        st.warning(
        ...
        st.error(
            f"❌ 你的出價比同棟最高價還貴 **{-potential_gap_wan:,.1f} 萬**！"
...
def _render_wash_tab() -> None:
    ...
    completed_steps = sum([step1, step2, step3, step4])

    st.markdown("---")
    st.markdown("##### 📊 過水進度")
    p1, p2, p3 = st.columns(3)
    p1.metric("完成步驟", f"{completed_steps} / 4")
    p2.metric("目前階段", WASH_STAGE_LABELS[completed_steps])
    p3.metric("安全等級", WASH_SAFETY_LABELS[completed_steps])
...
def render_chapter_4() -> None:
    ...
    兩個分頁：
        1. 整棟大廈價差／增貸計算機：(同棟最高單價 − 目標單價) × 坪數
        2. 科目四洗白過水追蹤器：四階段時間軸，滿 1 年才安全
    """
...
# 允許 `streamlit run chapter_4.py` 直接預覽此單一模組
