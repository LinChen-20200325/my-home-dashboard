"""進階策略模組：學長 — 極限議價與談判心理戰

獨立模組：提供 render_strategy_negotiation() 函式。
...
def _render_strategy_column(payload: dict) -> None:
    ...
    if title_lien.startswith("🔴"):
        st.success(
            ...
            f"你的首槍應該定錨在 **{ad_unit * 0.9:.1f} 萬／坪**，"
    ...
    with st.container():
        ...
        st.markdown(
            ...
            f"> 我願意付 **{ad_unit * 0.95:.1f} 萬／坪** 一次到位，"
            "請你幫我跟屋主談。」\n\n"
    ...
    # 戰術 3：收尾擋箭牌
    final_offer_wan = max(0.0, asking * 0.85 - 50.0)
    with st.container():
        ...
        st.markdown(
            ...
            "> 你讓 **50 萬**，我選你這間。」\n\n"
            f"📊 **以這案：建議最終出價 ≈ {final_offer_wan:,.1f} 萬**"
...
def render_strategy_negotiation() -> None:
    ...
    左右兩欄：
        左：談判籌碼輸入（5 維度：開價 / 廣告戶單價 / 待售時間 / 委託型態 / 謄本）
        右：戰術生成（底價推演引擎 + 三大議價戰術）
    """
    st.title("💼 極限議價與談判心理戰")
    st.info(
        "💡 **學長的實戰金句**：「**買房不是議價，是『獵屋』**。\n"
...
# 允許 `streamlit run strategy_negotiation.py` 直接預覽
