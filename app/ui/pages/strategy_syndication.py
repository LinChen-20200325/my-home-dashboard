"""Strategy Syndication UI — 合資套利與科目四無限槓桿（重構後）。

UI 層：純 widgets + 渲染；套利公式 + 防護鎖判定由 services.leverage。
"""
...
from app.models.constants import (
    BANK_REFINANCE_LTV,
    CASH_DEAL_DEFAULT_DISCOUNT,
    WAIT_MONTHS_FOR_REVAL,
)
...
def _render_arbitrage_tab() -> None:
    ...
    with col_a:
        market_price_wan = st.number_input(
            "🏠 目標物件市價（萬）",
            min_value=0.0, value=2_000.0, step=10.0, format="%.1f",
    ...
    with col_b:
        ...
        cash_deal_wan = st.number_input(
            "💵 現金買斷談判價（萬）",
            min_value=0.0, value=default_cash_deal, step=10.0, format="%.1f",
            help=f"預設為市價 {CASH_DEAL_DEFAULT_DISCOUNT * 100:.0f} 折"
                 "——這是學長的『破盤目標』。屋主急脫手時才可能接受。",
            key="syn_arb_cash_deal",
        )

    renovation_cost_wan = st.number_input(
        "🔨 預估輕裝修成本（萬）",
        min_value=0.0, value=80.0, step=10.0, format="%.1f",
        help="基本油漆、清潔、廚衛局部更新，**抬高銀行鑑價**用。",
        key="syn_arb_renovation",
    )

    result = calculate_syndication_arbitrage(
        market_price_wan=float(market_price_wan),
        cash_deal_price_wan=float(cash_deal_wan),
        renovation_cost_wan=float(renovation_cost_wan),
    )
    ...
    m1.metric(
        ...
        f"{result.bank_revaluation_wan:,.1f} 萬",
        delta=f"+{(result.bank_revaluation_wan - cash_deal_wan):,.1f} 萬 vs 你的買價",
        help="銀行半年後重估，會回歸市價而非你買到的破盤價。",
    )
    m2.metric(
        f"可增貸金額（鑑價 × {BANK_REFINANCE_LTV * 100:.0f}%）",
        f"{result.refinanced_amount_wan:,.1f} 萬",
        help="第二次貸款向銀行『把現金搬出來』的天花板。",
    )
    m3.metric(
        "🔨 裝修成本", f"{renovation_cost_wan:,.1f} 萬", delta_color="inverse",
    )

    st.markdown("---")
    st.metric(
        "🎯 預估『拿回本金 + 額外套出的現金』",
        f"{result.net_arbitrage_wan:,.1f} 萬",
        delta=f"vs 投入 {cash_deal_wan + renovation_cost_wan:,.1f} 萬",
        delta_color="normal" if result.is_profitable else "inverse",
    ...
    if result.is_profitable:
        st.info(
            ...
            f"鑑價回到 **{result.bank_revaluation_wan:,.1f} 萬**，"
            f"可貸出 **{result.refinanced_amount_wan:,.1f} 萬**\n"
            f"5. **本金 + 裝修費全數抽回**，還多套出 **{result.net_arbitrage_wan:,.1f} 萬**！\n\n"
        ...
        st.error(
            f"❌ **這筆生意不划算！** 淨套利 **{result.net_arbitrage_wan:,.1f} 萬**（負）。\n\n"
            "原因可能是：\n"
...
def _render_safeguard_tab() -> None:
    ...
    verdict = diagnose_syndication_safeguards(tuple(checked_flags))

    st.markdown("---")
    st.markdown("##### 📊 防護鎖到位度")
    p1, p2, p3 = st.columns(3)
    p1.metric("已到位", f"{verdict.locks_in_place} / {verdict.total_locks}")
    p2.metric("安全等級", _SAFETY_LADDER[verdict.locks_in_place])
    p3.metric(
        "可否簽合資協議",
        "✅ GO" if not verdict.is_naked else "❌ 停",
        delta_color="off",
    )
    st.progress(verdict.locks_in_place / verdict.total_locks)

    if verdict.is_naked:
        st.error(
            f"🚫 **資金裸奔中！** 還有 **{len(missing_labels)} 道法律防護鎖未上**：\n\n"
